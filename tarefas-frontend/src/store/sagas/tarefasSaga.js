...
function* fetchTarefasSaga() {
  try {
    const response = yield call(axios.get, 'https://etarefas-c64f7fa2b10d.herokuapp.com/api/tarefas');
...
function* addTarefaSaga(action) {
  try {
    ...
    const response = yield call(
      axios.post,
      'https://etarefas-c64f7fa2b10d.herokuapp.com/api/tarefas',
      formData,
      {
        headers: {
...
function* deleteTarefaSaga(action) {
  try {
    yield call(
      axios.delete,
      `https://etarefas-c64f7fa2b10d.herokuapp.com/api/tarefas/${action.payload}`
    );
...
function* updateTarefaSaga(action) {
  try {
    ...
    const response = yield call(
      axios.put,
      `https://etarefas-c64f7fa2b10d.herokuapp.com/api/tarefas/${id}`,
      formData,
      {
        headers: {
          'Content-Type': 'multipart/form-data',
