...
import {
  ...
  updateTarefaSuccess,
  updateTarefaFailure,
} from '../../../../tarefas-frontend/src/store/actions/tarefasActions';
import { API_URL } from '../../../../tarefas-frontend/src/api';  // Importação do API_URL

// Saga para buscar tarefas
function* fetchTarefasSaga() {
  try {
    const response = yield call(axios.get, `${API_URL}/tarefas`);
...
function* deleteTarefaSaga(action) {
  try {
    yield call(
      axios.delete,
      `${API_URL}/tarefas/${action.payload}`
    );
